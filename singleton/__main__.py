...
def main() -> None:
    ...
    for i in range(12):
        try:
            conn = DatabaseConnectionObjectPool(
                f"db{i+1}.example.com", 5432, f"app_db_{i+1}"
            )
            conn.connect()
    ...
    print(f"   Active instances: {DatabaseConnectionObjectPool.get_instance_count()}")
    ...
    new_conn = DatabaseConnectionObjectPool("new.db.com", 5432, "new_db")
    new_conn.connect()
